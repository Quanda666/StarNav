...
项目后端已提供空间读取接口，用于按访客权限返回空间列表：

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/spaces` | 按访客权限过滤返回空间列表（管理员可见全部） |

当前空间增删改接口处于稳定化冻结状态，调用会返回 `409`，前端空间切换与后台空间管理界面暂未开放。相关能力见 [第三阶段开发规划](./phase-3-multi-space-plan.md)。